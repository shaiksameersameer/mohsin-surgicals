...
useEffect(() => {
  ...
  const metaDescription = product.seo?.description || product.description?.replace(/(<([^>]+)>)/gi, "").substring(0, 160);
    ...
    <div className="bg-white min-h-screen pb-24 md:pb-12">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                 ...
                 {isRentalAvailable(product) && (
                   <button 
                     ...
                     className="hidden md:flex flex-1 py-3 px-6 rounded-lg font-bold text-base border-2 border-medical-primary text-medical-primary hover:bg-medical-light transition-all items-center justify-center gap-2"
                   >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 md:hidden z-40 flex items-center gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
        ...
        {isRentalAvailable(product) && (
          <button 
              onClick={() => setIsRentalModalOpen(true)}
              className="px-4 py-3 rounded-lg font-bold text-xs border border-medical-primary text-medical-primary"
          >
              RENT
          </button>
        )}
        <button 
            ...
            className={`flex-1 py-3 rounded-lg font-bold text-sm text-white ${product.inStock ? 'bg-medical-primary' : 'bg-gray-400'}`}
        >
            {product.inStock ? 'ADD TO CART' : 'OUT OF STOCK'}
        </button>
      </div>
